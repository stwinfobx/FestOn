...
class Inscricoes extends BaseController
{
		...
		**/
		if ($this->request->getPost()) {
			...
			if (!empty($fileInputLogotipo)) {
				$data_db['grp_logotipo'] = $fileInputLogotipo;
			}
    ...
    **/
    if ($this->request->getPost()) {
        ...
        // Verificar se existem coreografias cadastradas
        $this->grpMD->from('tbl_grupos As GRP', true)
            ->select('GREVT.grevt_id')
            ->select('GRP.*')
            ->select('EVENT.*')
        ...
        if ($query_grupo_evt && $query_grupo_evt->resultID->num_rows >= 1) {
            ...
            // Verificar se existem coreografias cadastradas
            $query_coreografias = $this->corgfMD
                ->where('insti_id', $insti_id)
                ->where('grp_id', $grp_id)
                ->where('corgf_ativo', 1)
                ->get();
                
            if (!$query_coreografias || $query_coreografias->resultID->num_rows === 0) {
                // Redirecionar de volta com mensagem de erro
                session()->setFlashdata('error', 'É necessário cadastrar pelo menos uma coreografia para continuar.');
                return $this->response->redirect(site_url('inscricoes/coreografias/' . $grevt_hashkey));
            }
        }
        // ============ FIM DA VALIDAÇÃO ============
        
        // Se tem coreografias, continuar com o processamento normal
		...
		if ($query_grupo_evt && $query_grupo_evt->resultID->num_rows >= 1) {
			...
			**/
			...
			if ($query_event_config && $query_event_config->resultID->num_rows >= 1) {
				...
				if (!is_array($arr_evcfg_forma_cobranca)) {
					$arr_evcfg_forma_cobranca = [];
				}
	...
	{
		log_message('info', 'MÉTODO DOACOES CHAMADO - grevt_hashkey: ' . $grevt_hashkey);
		log_message('info', 'REQUEST METHOD: ' . $this->request->getMethod());
		log_message('info', 'REQUEST POST DATA: ' . json_encode($this->request->getPost()));

		if (!session()->get('isLoggedInUserInscricao') == TRUE) {
			log_message('warning', 'Usuário não logado - redirecionando para erro');
			return $this->response->redirect(site_url('inscricoes/error'));
		}
		$user_id = (int)session()->get('inscUser_id');
		 ...
		 * -------------------------------------------------------------
		**/
		// Verificar se é POST com confirmação de finalização
		if ($this->request->getPost() && $this->request->getPost('confirmar_finalizacao')) {
			log_message('info', 'POST COM CONFIRMAÇÃO DETECTADO - Iniciando finalização da inscrição para grevt_hashkey: ' . $grevt_hashkey);
			log_message('info', 'Dados POST recebidos: ' . json_encode($this->request->getPost()));

			try {
				// Enviar email de confirmação antes de redirecionar
				$this->enviarEmailConfirmacaoInscricao($grevt_hashkey);
				log_message('info', 'Email/webhook enviado com sucesso');

				// Atualizar status da inscrição para concluída
				$this->atualizarStatusInscricao($grevt_hashkey);
				log_message('info', 'Status atualizado para concluída');

				$redirect_url = site_url('inscricoes/status/' . $grevt_hashkey);
				log_message('info', 'REDIRECIONANDO para: ' . $redirect_url);

				// Garantir que o redirecionamento funcione
				return redirect()->to($redirect_url);
			} catch (Exception $e) {
				log_message('error', 'Erro na finalização da inscrição: ' . $e->getMessage());

				// Mesmo com erro, redirecionar para status
				$redirect_url = site_url('inscricoes/status/' . $grevt_hashkey);
				log_message('info', 'Redirecionando mesmo com erro para: ' . $redirect_url);
				return redirect()->to($redirect_url);
			}
		...
		**/
		...
		$this->data['event_hashkey'] = $event_hashkey ?? '';
		$this->data['list_rs_coreografos'] = $this->data['rs_coreografos'] ?? [];
		$this->data['list_rs_formatos'] = $this->data['rs_formatos'] ?? [];
		$this->data['list_rs_categorias'] = $this->data['list_rs_categorias'] ?? [];
		$this->data['evcfg_config_limites'] = (isset($this->data['rs_event_config']) && $this->data['rs_event_config']) ? $this->data['rs_event_config']->evcfg_config_limites ?? [] : [];
		$this->data['evcfg_config_infos'] = (isset($this->data['rs_event_config']) && $this->data['rs_event_config']) ? $this->data['rs_event_config']->evcfg_config_infos ?? [] : [];
		$this->data['rs_corgf_cadastradas'] = $this->data['rs_corgf_cadastradas'] ?? [];
		$this->data['lista_de_coreografias'] = $this->data['lista_de_coreografias'] ?? [];
		$this->data['elenco_geral'] = $this->data['elenco_geral'] ?? [];
		$this->data['coreografias_geral'] = $this->data['coreografias_geral'] ?? [];
		$this->data['doacoes_geral'] = $this->data['doacoes_geral'] ?? [];
		...
		if ($this->request->getPost()) {
			...
			**/
			if ($prosseguir == true) {
				$validateFields = [
					'user_nome' => [
						'label' => 'user_nome', 'rules' => 'required',
						'errors' => [
					...
					'user_email' => [
						'label' => 'user_email', 'rules' => 'required',
						'errors' => [
							'required' => 'Preencha corretamente {field}.',
							//'validar_cpf' => 'O {field} informado é inválido.',
						],
					...
					'user_senha' => [
						'label' => 'user_senha', 'rules' => 'required',
						'errors' => [
							'required' => 'Preencha corretamente {field}.',
							//'validar_cpf' => 'O {field} informado é inválido.',
						],
	...
	{
		// Buscar grupos com informações de status da inscrição
		$this->grpMD->from('tbl_grupos AS GRP', true)
			->select('GRP.*')
			->select('GREVT.grevt_hashkey, GREVT.grevt_ativo')
			->join('tbl_grupos_x_eventos AS GREVT', 'GREVT.grp_id = GRP.grp_id', 'LEFT')
			->where('GRP.user_id', (int)session()->get('inscUser_id'))
				...
				**/
					...
					*/
						...
						**/
							...
							if (!empty($partc_file_foto)) {
								$data_cadastro_db['cad_file_foto'] = $partc_file_foto;
							}
			...
			case "SALVAR-ELENCO-COREOGRAFIA":
				...
				**/
					...
					**/
						...
						if (!empty($fileInputMusica)) {
							$data_coreografia_db['corgf_musica_file'] = $fileInputMusica;
						}
					...
					**/
						$json_data_A = [];
						$json_data_B = [];
						if (!empty($elenco_coreografos_json)) {
							$json_data_A = json_decode($elenco_coreografos_json);
						}
						if (!empty($elenco_bailarinos_json)) {
							$json_data_B = json_decode($elenco_bailarinos_json);
						}
			...
			if (in_array('por_participante', $forma_cobranca)) {
				...
				if ($query_valor_por_funcao && $query_valor_por_funcao->resultID->num_rows >= 1) {
					...
					if (in_array('doacao', $forma_cobranca)) {
						...
						if ($evvlr_data_limite === '0000-00-00' || empty($evvlr_data_limite)) {
							$evvlr_data_limite = '';
						}
	...
	private function enviarEmailConfirmacaoInscricao($grevt_hashkey)
	{
		...
		if ($query_grupo && $query_grupo->resultID->num_rows >= 1) {
			...
			// Buscar diretor do grupo
			$diretor_email = '';
			$this->partcMD->from('tbl_participantes AS P', true)
				->select('P.partc_email')
				->join('tbl_participantes_x_grupos AS PG', 'PG.partc_id = P.partc_id', 'INNER')
				->join('tbl_funcoes AS F', 'F.func_id = PG.func_id', 'INNER')
				->where('PG.grevt_id', $grupo->grevt_id)
				->where('F.func_titulo', 'Diretor')
				->limit(1);
	...
	/**
	 * Envia dados para webhook quando em localhost
	 */
	private function enviarWebhookConfirmacao($emails_destino, $email_body, $data_fields)
	{
		$webhook_url = 'https://leowebhook.techcorps.com.br/webhook/371511f3-4a5a-44fd-b30d-fa51a24f58c1';
		...
		$webhook_data = [
			...
			'email_subject' => 'Confirmação de Inscrição - Dança Carajás Festival 2025',
			'email_body' => $email_body,
			'grupo_data' => $data_fields,
			'webhook_received_at' => date('c')
		];

		// Log dos dados que serão enviados
		log_message('info', 'Enviando webhook para: ' . $webhook_url);
		log_message('info', 'Dados do webhook: ' . json_encode($webhook_data));

		// Configurar cURL para enviar para webhook
		$ch = curl_init();
		...
		curl_setopt($ch, CURLOPT_HTTPHEADER, [
			...
			'User-Agent: Dança-Carajás-Festival-2025/1.0'
		]);
		...
		if ($error) {
			log_message('error', 'Webhook cURL Error: ' . $error);
			log_message('error', 'Webhook cURL Info: ' . json_encode($info));
		} else {
			log_message('info', 'Webhook sent successfully!');
			log_message('info', 'HTTP Code: ' . $http_code);
			log_message('info', 'Response: ' . $response);
			log_message('info', 'cURL Info: ' . json_encode($info));
		}

		// Para debug adicional, vamos também fazer echo se estiver em localhost
		if (isset($_SERVER['HTTP_HOST']) && strpos($_SERVER['HTTP_HOST'], 'localhost') !== false) {
			...
			echo "<!-- END WEBHOOK DEBUG -->\n";
		}
	}

	/**
	 * Atualiza status da inscrição para concluída
	 */
	private function atualizarStatusInscricaoOld($grevt_hashkey)
	{
		try {
			// Buscar o registro primeiro para obter o ID
			$query = $this->grevtMD->where('grevt_hashkey', $grevt_hashkey)->get();

			if ($query && $query->resultID->num_rows >= 1) {
				$registro = $query->getRow();
				$grevt_id = $registro->grevt_id;

				// Dados para atualização
				$update_data = [
					'grevt_ativo' => 2, // 2 = Inscrição Concluída
					'grevt_dte_alteracao' => date('Y-m-d H:i:s')
				];

				// Atualizar usando o ID específico
				$result = $this->grevtMD->update($grevt_id, $update_data);

				if ($result) {
					log_message('info', 'Status da inscrição atualizado com sucesso para grevt_id: ' . $grevt_id);
				} else {
					log_message('error', 'Erro ao atualizar status da inscrição para grevt_id: ' . $grevt_id);
				}
			} else {
				log_message('warning', 'Registro não encontrado para grevt_hashkey: ' . $grevt_hashkey);
			}
		} catch (Exception $e) {
			log_message('error', 'Erro na atualização do status: ' . $e->getMessage());
		}
	}

    private function atualizarStatusInscricao($grevt_hashkey)
    {
        try {
            // Buscar o registro primeiro para obter o ID
            $query = $this->grevtMD->where('grevt_hashkey', $grevt_hashkey)->get();
            ...
            if ($query && $query->resultID->num_rows >= 1) {
                $registro = $query->getRow();
                $grevt_id = $registro->grevt_id;
                $grp_id = $registro->grp_id;
    
                // ⭐⭐ 1. INSTÂNCIA DIRETA DO DATABASE
                $db = \Config\Database::connect();
                $sql_grupo = "UPDATE tbl_grupos SET grp_status = 1, grp_dte_alteracao = NOW() WHERE grp_id = ?";
                $result_grupo = $db->query($sql_grupo, [$grp_id]);
    
                // ⭐⭐ 2. MANTÉM O MODEL PARA tbl_grupos_x_eventos (que já funcionava)
                $update_data = [
                    'grevt_ativo' => 2, // 2 = Inscrição Concluída
                    'grevt_dte_alteracao' => date('Y-m-d H:i:s')
                ];
                $result_inscricao = $this->grevtMD->update($grevt_id, $update_data);
    
                if ($result_inscricao && $result_grupo) {
                    log_message('info', 'Status da inscrição e grupo atualizados com sucesso - grevt_id: ' . $grevt_id . ', grp_id: ' . $grp_id);
                    return true;
                } else {
                    log_message('error', 'Erro ao atualizar status para grevt_id: ' . $grevt_id);
                    return false;
                }
            } else {
                log_message('warning', 'Registro não encontrado para grevt_hashkey: ' . $grevt_hashkey);
	...
	/**
	 * Função de teste para webhook (apenas para debug)
	 */
	public function testar_webhook()
	{
		// Apenas para testes em localhost
		$http_host = $_SERVER['HTTP_HOST'] ?? '';
		$is_localhost = in_array($http_host, ['localhost', '127.0.0.1']) ||
			strpos($http_host, 'localhost') !== false ||
			strpos($http_host, '127.0.0.1') !== false ||
		...
		if (!$is_localhost) {
			echo "Esta função só funciona em localhost para testes!<br>";
			echo "HTTP_HOST atual: " . $http_host;
			return;
		}

		$dados_teste = [
			'emails_destino' => ['inscricoes@dancacarajas.com.br', 'teste@exemplo.com'],
			'email_body' => '<h1>Teste de Webhook</h1><p>Este é um teste do webhook!</p>',
			'data_fields' => [
				'grevt_hashkey' => 'teste-123',
				'grp_titulo' => 'Grupo Teste',
				'grp_end_cidade' => 'Cidade Teste',
				'grp_end_estado' => 'Estado Teste'
			]
		];

		echo "<h2>Testando Webhook...</h2>";
		echo "<p><strong>HTTP_HOST:</strong> " . $http_host . "</p>";
		echo "<p><strong>IS_LOCALHOST:</strong> " . ($is_localhost ? 'SIM' : 'NÃO') . "</p>";
		echo "<p>Dados que serão enviados:</p>";
		echo "<pre>" . json_encode($dados_teste, JSON_PRETTY_PRINT) . "</pre>";

		$this->enviarWebhookConfirmacao($dados_teste['emails_destino'], $dados_teste['email_body'], $dados_teste['data_fields']);

		echo "<p><strong>Webhook enviado! Verifique os logs e seu workflow.</strong></p>";
		echo "<p><a href='" . site_url('inscricoes/meus-grupos') . "'>Voltar para Meus Grupos</a></p>";
	}

	/**
	 * Função de teste para redirecionamento
	 */
	public function testar_redirect()
	{
		$grevt_hashkey = '5188a0ae965eb2cdc2036958f82b2331'; // Hash da sua inscrição

		echo "<h2>Testando Redirecionamento...</h2>";
		echo "<p>Hash da inscrição: " . $grevt_hashkey . "</p>";
		echo "<p>URL de destino: " . site_url('inscricoes/status/' . $grevt_hashkey) . "</p>";

		echo "<p><a href='" . site_url('inscricoes/status/' . $grevt_hashkey) . "' target='_blank'>Clique aqui para testar o redirecionamento</a></p>";

		echo "<p><strong>Se este link funcionar, o problema está no JavaScript/Formulário</strong></p>";
		echo "<p><a href='" . site_url('inscricoes/meus-grupos') . "'>Voltar para Meus Grupos</a></p>";
	}
}
